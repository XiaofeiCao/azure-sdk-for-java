...
public final class ResourceDetailsListByPoolMockTests {
    ...
    public void testListByPool() throws Exception {
        String responseStr
            = "{\"value\":[{\"properties\":{\"status\":\"PendingReturn\",\"image\":\"orzihle\",\"imageVersion\":\"sjswsrms\"},\"id\":\"zrpzb\",\"name\":\"hckqqzqioxiy\",\"type\":\"uiizynke\"}]}";
        ...
        PagedIterable<ResourceDetailsObject> response
            = manager.resourceDetails().listByPool("n", "jphuopxodlqi", com.azure.core.util.Context.NONE);

        Assertions.assertEquals(ResourceStatus.PENDING_RETURN, response.iterator().next().properties().status());
        Assertions.assertEquals("orzihle", response.iterator().next().properties().image());
        Assertions.assertEquals("sjswsrms", response.iterator().next().properties().imageVersion());
    }
}
